...
mkdir -p "$SCRATCHDIR"
mkdir -p "$SCRATCHDIR/optimus-amzn"

cp -r /gpfs/project/$USER/cooplio/* $SCRATCHDIR/.
cp -r /gpfs/project/$USER/models/optimus-amzn/* $SCRATCHDIR/optimus-amzn/.

cp /gpfs/project/$USER/cooplio/data/amzn/dev.json $SCRATCHDIR/dev.json
...
python evaluatePPLM.py config/optimus/amznKeyword.jsonnet -s log/optimus/amzn/ex1 >>  $LOGFILE

cp $SCRATCHDIR/eval_results.json /gpfs/project/$USER/eval_results_normal.json
