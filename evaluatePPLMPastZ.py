...
def perturb_past(
    ...
    grad_clean = np.zeros(past_key_values.shape).astype("float32")
    #print(grad_accumulator)
    ...
    curr = to_var(torch.from_numpy(past_key_values.cpu().detach().numpy()),requires_grad=True, device=device)
    for i in range(num_iterations):
        ...
        #perturbed_past = list(map(add, past_key_values, curr_perturbation))
        #print(curr)
        ...
        all_logits, _, all_hidden = model(**{
            "input_ids": last,
            "past_key_values": (curr,),
            "latent_as_gpt_memory": True,
            "latent_as_gpt_emb": True,
        }, return_dict=False, output_hidden_states=True)  # (batch_size * num_beams, cur_len, vocab_size)
        ...
        curr = to_var(torch.from_numpy(curr.cpu().detach().numpy()+grad),requires_grad=True, device=device)
    ...
    #    for p_ in grad_accumulator
    #]
    pert_past = curr#list(map(add, past, grad_accumulator))
...
def _generate_beam_search(
        ...
        while cur_len < max_length:
            ...
            last = input_ids[:,-1:]
            if not first:
              pert_logits, past, pert_all_hidden = model(**{
                  "input_ids": input_ids,
                  "past_key_values": (pert_past,),
                  "latent_as_gpt_memory": True,
                  "latent_as_gpt_emb": True,
              },use_cache=True, return_dict=False, output_hidden_states=True)  ## REALLY OVERWRITE PAST ???
            ...
            if do_sample:
                ...
                if perturb and not first:
                  ...
                  #print(_pert_next_tokens)
                  #print(next_tokens)
                  #if torch.equal(next_tokens,_pert_next_tokens):
                  #  print(Fore.CYAN + bcolors.BOLD+ "--diff--"+bcolors.ENDC)
...
def evaluatePPLM(rev, summ, tokenizerGPT, vae, gamma=1.5, gm_scale=0.5, kl_scale=0.01, stepsize=0.05, num_return_sequences=1):
    torch.cuda.empty_cache() 
    one_hot_bows_vectors = extractVectorY(rev, tokenizerGPT, 500, "cuda")
...
@click.command()
@click.argument('gamma', type=float, default=1.5)
@click.argument('gm_scale', type=float, default=0.5)
@click.argument('kl_scale', type=float, default=0.01)
@click.argument('stepsize', type=float, default=0.02)
@click.argument('num_return_sequences', type=float, default=1)
def main(gamma=1.5, gm_scale=0.5, kl_scale=0.01, stepsize=0.05, num_return_sequences=1):
    print("Evaluating...")
    ...
    model_name: str = "models/optimus-amzn"#"megagonlabs/optimus-amzn" #"./" #"megagonlabs/optimus-amzn"  # or "megagonlabs/bimeanvae-amzn", "megagonlabs/optimus-yelp", "megagonlabs/optimus-amzn"
    vae = VAE(model_name)
    ...
    with open('eval_results.json', 'w') as outfile:
        data = {"data":output_eval,"options":{"gamma":gamma,"gm_scale":gm_scale,"kl_scale":kl_scale,"stepsize":stepsize,"num_return_sequences":num_return_sequences}}
